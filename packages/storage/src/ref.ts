...
class ReferenceStore implements AsyncReadable<RequestInit> {
	...
	static fromSpec(spec: Record<string, unknown>, opts?: ReferenceStoreOptions): ReferenceStore {
		// @ts-expect-error - TS doesn't like the type of `parse`
		let refs = parse(spec);
		return new ReferenceStore(refs, opts);
	}

	static async fromUrl(url: string | URL, opts?: ReferenceStoreOptions): Promise<ReferenceStore> {
