...
test("BoolArray", () => {
	...
	expect([false, false, false, false, false]).toBe(Array.from(arr));
	expect(arr.get(1)).toBe(false);
	arr.set(1, true);
	expect(arr.get(1)).toBe(true);
	arr.set(3, true);
	expect([false, true, false, true, false]).toBe(Array.from(arr));
	...
	expect([true, true, false, false]).toBe(Array.from(arr));

	arr.fill(true);
	expect([true, true, true, true]).toBe(Array.from(arr));
});
...
test("ByteStringArray", () => {
	...
	expect(arr.length).toBe(5);
	expect(["Hello", "world!", "", "", ""]).toBe(Array.from(arr));
	arr.set(4, "test");
	expect(["Hello", "world!", "", "", "test"]).toBe(Array.from(arr));
...
test("UnicodeStringArray - special characters", () => {
	...
	expect(arr.length).toBe(3);
	expect(["¡Hola mundo!", "Hej Världen!", "Xin chào thế giới"]).toBe(Array.from(arr));
	arr.set(2, "test");
	expect(["¡Hola mundo!", "Hej Världen!", "test"]).toBe(Array.from(arr));
});
