...
import type * as core from "@zarrita/core/types";
...
let root = path.resolve(__dirname, "../../v2/__tests__/data/data.zarr");
...
describe("ndarray", () => {
	...
	it("reads 1d.contiguous.zlib.i2", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.zlib.i2") as core.Array<"<i2">;
	...
	it("reads 1d.contiguous.blosc.i2", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.blosc.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array([1, 2, 3, 4]));
		expect(res.shape).toStrictEqual([4]);
	});
	...
	it("reads 1d.contiguous.lz4.i2", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.lz4.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array([1, 2, 3, 4]));
		expect(res.shape).toStrictEqual([4]);
	});
	...
	it("reads 1d.contiguous.zstd.i2", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.zstd.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array([1, 2, 3, 4]));
		expect(res.shape).toStrictEqual([4]);
	});
	...
	it("reads 1d.contiguous.raw.i2", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.raw.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array([1, 2, 3, 4]));
		expect(res.shape).toStrictEqual([4]);
	});
	...
	it("reads 1d.contiguous.i4", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.i4") as core.Array<"<i4">;
		let res = await get(arr);
	...
	it("reads 1d.contiguous.u1", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.u1") as core.Array<"|u1">;
		let res = await get(arr);
	...
	it("reads 1d.contiguous.f4.le", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.f4.le") as core.Array<"<f4">;
		let res = await get(arr);
	...
	it("reads 1d.contiguous.f4.be", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.f4.be") as core.Array<">f4">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Float32Array([-1000.5, 0, 1000.5, 0]));
		expect(res.shape).toStrictEqual([4]);
	});
	...
	it("reads 1d.contiguous.f8", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.f8") as core.Array<"<f8">;
		let res = await get(arr);
	...
	it("reads 1d.contiguous.U13.le", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.U13.le") as unknown as core.Array<
			"<U13"
		>;
	...
	it("reads 1d.contiguous.U13.be", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.U13.be") as unknown as core.Array<
			">U13"
		>;
		let res = await get(arr);
		expect(res.data).toBeInstanceOf(UnicodeStringArray);
	...
	it("reads 1d.contiguous.U7", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.U7") as unknown as core.Array<"<U7">;
		let res = await get(arr);
		expect(res.data).toBeInstanceOf(UnicodeStringArray);
		expect(Array.from(res.data)).toStrictEqual(["a", "b", "cc", "d"]);
		expect(res.shape).toStrictEqual([4]);
	...
	it("reads 1d.contiguous.S7", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.S7") as unknown as core.Array<"|S7">;
		let res = await get(arr);
	...
	it("reads 1d.contiguous.b1", async () => {
		let arr = await zarr.get_array(store, "/1d.contiguous.b1") as core.Array<"|b1">;
		let res = await get(arr);
	...
	it("reads 2d.contiguous.i2", async () => {
		let arr = await zarr.get_array(store, "/2d.contiguous.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array([1, 2, 3, 4]));
	...
	it("reads 3d.contiguous.i2", async () => {
		let arr = await zarr.get_array(store, "/3d.contiguous.i2") as core.Array<"<i2">;
		let res = await get(arr);
	...
	it("reads 1d.chunked.i2", async () => {
		let arr = await zarr.get_array(store, "/1d.chunked.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array([1, 2, 3, 4]));
		expect(res.shape).toStrictEqual([4]);
	});
	...
	it("reads 1d.chunked.ragged.i2", async () => {
		let arr = await zarr.get_array(store, "/1d.chunked.ragged.i2") as core.Array<"<i2">;
		let res = await get(arr);
	...
	it("reads 2d.chunked.i2", async () => {
		let arr = await zarr.get_array(store, "/2d.chunked.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array([1, 2, 3, 4]));
		expect(res.shape).toStrictEqual([2, 2]);
	});
	...
	it("reads 2d.chunked.U7", async () => {
		let arr = await zarr.get_array(store, "/2d.chunked.U7") as unknown as core.Array<"<U7">;
		let res = await get(arr as any);
		expect(Array.from(res.data as any)).toStrictEqual(["a", "b", "cc", "d"]);
		expect(res.shape).toStrictEqual([2, 2]);
	});
	...
	it("reads 2d.chunked.ragged.i2", async () => {
		let arr = await zarr.get_array(store, "/2d.chunked.ragged.i2") as core.Array<"<i2">;
		let res = await get(arr);
	...
	it("reads 3d.chunked.i2", async () => {
		let arr = await zarr.get_array(store, "/3d.chunked.i2") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array(range(27)));
		expect(res.shape).toStrictEqual([3, 3, 3]);
	});
	...
	it("reads 3d.chunked.mixed.i2.C", async () => {
		let arr = await zarr.get_array(store, "/3d.chunked.mixed.i2.C") as core.Array<"<i2">;
		let res = await get(arr);
		expect(res.data).toStrictEqual(new Int16Array(range(27)));
		expect(res.shape).toStrictEqual([3, 3, 3]);
	...
	it("reads 3d.chunked.mixed.i2.F", async () => {
		let arr = await zarr.get_array(store, "/3d.chunked.mixed.i2.F") as core.Array<"<i2">;
		let res = await get(arr);
		// deno-fmt-ignore
