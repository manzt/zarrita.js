...
export async function _set_selection(indexer, value) {
    ...
    if (!(typeof value === 'number')) {
        assert(value.data instanceof this.TypedArray, `Incorrect dtype, tried to assign ${value.data.constructor.name} to ${this.dtype} (${this.TypedArray.constructor.name}).`);
    }
    ...
    constructor({ selection, shape, chunk_shape, }) {
        ...
        this.dim_indexers = selection.map((dim_sel, i) => {
            if (typeof dim_sel === 'number') {
                ...
                return new _SliceDimIndexer({ dim_sel: dim_sel, dim_len: shape[i], dim_chunk_len: chunk_shape[i] });
            }
