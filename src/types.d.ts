...
export type Endianness<Dtype extends DataType> = Dtype extends `${infer E}${infer _}` ? E : never;
export type DataTypeMappingKey<Dtype extends DataType> = Dtype extends `${infer _}${infer Key}` ? Key : never;

export type TypedArrayConstructor<Dtype extends DataType> = DataTypeMapping[DataTypeMappingKey<Dtype>];
export type TypedArray<Dtype extends DataType> = InstanceType<TypedArrayConstructor<Dtype>>;
...
type ZarrArray<D extends DataType, S extends Store> = import("./hierarchy").ZarrArray<D, S>;
type ExplicitGroup<S extends Store, H extends Hierarchy<S>> = import("./hierarchy").ExplicitGroup<S, H>;
type ImplicitGroup<S extends Store, H extends Hierarchy<S>> = import("./hierarchy").ImplicitGroup<S, H>;
...
export interface Hierarchy<Store extends SyncStore | AsyncStore> {
  ...
  get_explicit_group(path: string): Promise<ExplicitGroup<Store, Hierarchy<Store>>>;
  ...
  // write
  create_group(path: string, props?: { attrs?: Attrs }): Promise<ExplicitGroup<Store, Hierarchy<Store>>>;
  create_array<
    Dtype extends DataType
  >(path: string, props: Omit<ArrayAttributes<Dtype>, 'path' | 'store'>): Promise<ZarrArray<Dtype, Store>>;
}
